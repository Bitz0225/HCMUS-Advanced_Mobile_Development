          ...
          body: SingleChildScrollView(
            child: Container(
              ...
              child: Column(children: [
                ...
                state.bookedScheduleOutput?.data?.rows?.isEmpty ?? true
                    ? Column(
                        children: [
                          const SizedBox(height: 16),
                          const Text(
                    ...
                    : ListView.builder(
                        ...
                        itemCount:
                            state.bookedScheduleOutput?.data?.rows?.length ?? 0,
                        itemBuilder: (context, index) {
                          return Column(
                            children: [
                              const SizedBox(height: 16),
                              ScheduleItem(
                                bookedSchedule: state
                                    .bookedScheduleOutput?.data?.rows?[index],
                              ),
                            ],
                          );
                        },
